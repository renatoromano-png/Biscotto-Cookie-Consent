<?php
/**
 * Standalone test for Biscotto_Cookie_Database — no WordPress bootstrap.
 * Run: php tests/test-cookie-database.php
 */

define( 'ABSPATH', __DIR__ . '/' ); // satisfies the class file's guard

require_once __DIR__ . '/../packages/wordpress/includes/class-biscotto-cookie-database.php';
...
// --- Fixture CSV -----------------------------------------------------
$fixture = sys_get_temp_dir() . '/biscotto-test-ocd.csv';
$csv     = <<<CSV
...
$index = Biscotto_Cookie_Database::build_index( $fixture );

// --- Exact cookie match ------------------------------------------------
$m = Biscotto_Cookie_Database::lookup_cookie( '_ga_test', $index );
...
$m = Biscotto_Cookie_Database::lookup_cookie( 'TestConsentBulk-XYZ123', $index );
...
// --- No match ------------------------------------------------------------
$m = Biscotto_Cookie_Database::lookup_cookie( 'totally_unknown_cookie', $index );
ck_assert( null === $m, 'unknown cookie returns null' );

// --- Domain match, with "(3rd party)" suffix cleaned --------------------
$m = Biscotto_Cookie_Database::lookup_domain( 'fonts.testcdn.com', $index );
...
$m = Biscotto_Cookie_Database::lookup_domain( 'assets.fonts.testcdn.com', $index );
ck_assert( null !== $m, 'subdomain matches a bare domain entry' );

// --- Security -> necessary -----------------------------------------------
$m = Biscotto_Cookie_Database::lookup_cookie( '__test_csrf', $index );
ck_assert( null !== $m && 'necessary' === $m['category'], 'Security category maps to necessary' );

// --- Literal "Necessary" category value ---------------------------------
$m = Biscotto_Cookie_Database::lookup_cookie( 'legacy_session', $index );
ck_assert( null !== $m && 'necessary' === $m['category'], 'literal "Necessary" category maps to necessary' );

// --- map_category direct ---------------------------------------------------
ck_assert( 'marketing' === Biscotto_Cookie_Database::map_category( 'Marketing' ), 'map_category Marketing' );
ck_assert( 'necessary' === Biscotto_Cookie_Database::map_category( 'Something Unknown' ), 'map_category unknown falls back to necessary' );
